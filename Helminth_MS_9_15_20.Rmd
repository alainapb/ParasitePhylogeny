...
gmpd_parasite_data <- read.csv("data/GMPD_parasite_traits.csv")
...
saveRDS(gmpd_host_pubs, "data/GMPD_host_pubs.RDS")
saveRDS(gmpd_para_pubs, "data/GMPD_para_pubs.RDS")
saveRDS(nearctic_host_pubs, "data/Nearctic_host_pubs.RDS")
saveRDS(nearctic_para_pubs, "data/Nearctic_para_pubs.RDS")
```

```{r load_research_effort_data, include = FALSE}
gmpd_host_pubs <- readRDS("data/GMPD_host_pubs.RDS")
gmpd_para_pubs <- readRDS("data/GMPD_para_pubs.RDS")
nearctic_host_pubs <- readRDS("data/Nearctic_host_pubs.RDS")
nearctic_para_pubs <- readRDS("data/Nearctic_para_pubs.RDS")
```
...
#save matrix
write.csv(nearctic_df,file = "data/association_matrix_nearctic.csv", row.names = TRUE, col.names = TRUE)
...
#View residual MPD on phylogeny
```{r figures, echo = FALSE}
#NEARCTIC DATASET
res_mpd_nearctic<-gls_host_pubs_mpd_nearctic$residuals
names(res_mpd_nearctic)<-nearctic_parasite_tree$tip.label

#view mpd with single host species = 0 on the tree
fig1<-contMap(nearctic_parasite_tree, res_mpd_nearctic, fsize= .3, lwd=1, leg.text="x", type="fan") 

#color friendly
library(viridis)
n_cols <- n_distinct(res_mpd_nearctic)
contmap_obj_viridis <- setMap(fig1, viridis(n_cols))

# Plot the mapped characters with the new colors
plot(
  contmap_obj_viridis, 
  type="fan", 
  legend = 0.7*max(nodeHeights(nearctic_parasite_tree)),
  fsize = c(0.3, 0.5),
  lwd = 7)

#GMPD
res_mpd_gmpd<-gls_host_pubs_mpd_gmpd$residuals
names(res_mpd_gmpd)<-gmpd_parasite_tree$tip.label

#view mpd with single host species = 0 on the tree
fig2 <-contMap(gmpd_parasite_tree, res_mpd_gmpd, fsize= .3,lwd=2, type="fan") 
n_cols2 <- n_distinct(res_mpd_gmpd)
contmap_obj_viridis2 <- setMap(fig2, viridis(n_cols2))

# Plot the mapped characters with the new colors
plot(
  contmap_obj_viridis2, 
  type="fan", 
  legend = 0.7*max(nodeHeights(gmpd_parasite_tree)),
  fsize = c(0.3, 0.5),
  lwd = 4)
```
...
saveRDS(nearctic_phyla_data, "data/nearctic_phyla_data.RDS")
saveRDS(gmpd_phyla_data, "data/gmpd_phyla_data.RDS")
```

#Neartic ouch Models of the residuals of gls(mpd ~ host pubs) by Phyla
```{r nearctic_evo_phyla, echo = FALSE}
nearctic_phyla_data <- readRDS("data/nearctic_phyla_data.RDS")
...
#save for supplement
saveRDS(nearctic_ou1_ouch, file ="ouch_results/nearctic_ou1_ouch")
saveRDS(nearctic_bm_ouch, file = "ouch_results/nearctic_bm_ouch")
saveRDS(nearctic_oum_ouch, file ="ouch_results/nearctic_oum_ouch")
```

#GMPD ouch Models of the residuals of gls(mpd ~ host pubs) by Phyla
```{r gmpd_evo_phyla, echo = FALSE}
gmpd_phyla_data <- readRDS("data/gmpd_phyla_data.RDS")
...
#save for supplement 
saveRDS(gmpd_bm_ouch, file = "ouch_results/gmpd_bm_ouch")
saveRDS(gmpd_ou1_ouch, file = "ouch_results/gmpd_ou1_ouch")
saveRDS(gmpd_oum_ouch, file = "ouch_results/gmpd_oum_ouch")
```


#Nearctic ouch Models of the residuals of gls(mpd ~ host pubs by Class)
```{r nearctic_evo_class, echo = FALSE}
## remake tree to be able to use ouch rather than OUwie
nearctic_class_ouchtree <- ape2ouch(nearctic_parasite_tree) ## convert to ouchtree object
## get the mpd data organized properly for ouch
nearctic_class_ouchdata <- rep(NA, length=nearctic_class_ouchtree@nnodes)
for (i in 1:length(nearctic_phyla_data$species))
  nearctic_class_ouchdata[which(nearctic_class_ouchtree@nodelabels==nearctic_phyla_data$species[i])] <- nearctic_phyla_data$trait[i]
nearctic_class_ouchdata <- as.data.frame(nearctic_class_ouchdata)

#Classes = "Palaeacanthocephala" "Archiacanthocephala" "Chromadorea" "Enoplea" "Trematoda"  "Cestoda"   
## Find the node that corresponds to the most recent common ancestor of all Palaeacanthocephala
## which nodes in the ouchtree correspond to the Plats
nearctic_class_ouchtree@nodelabels%in%nearctic_phyla_data$species[which(nearctic_phyla_data$regime=="Palaeacanthocephala")] %>% which -> palae
## get their lineages and find the nodes that are in common across all lineages
nearctic_class_ouchtree@lineages[palae] -> palae_lineages
palae_lineages[[1]][sapply(palae_lineages[[1]], function(n) lapply(palae_lineages, function(l) n%in%l) %>% unlist %>% all) %>% which %>% min] ## 85

## paint the taxonomy-based regimes onto the tree
paint(nearctic_class_ouchtree, subtree=c("1" = "Cestoda","89" = "Trematoda", "85"="Palaeacanthocephala","81" = "Enoplea","81"="Chromadorea", "82"="Archiacanthocephala"), branch=c("1"="Cestoda")) -> nearctic_oum_class_regimes
## paint a single global regime onto the tree
paint(nearctic_class_ouchtree, subtree=c("1"="global"), branch=c("1"="global")) -> nearctic_ou1_class_regimes
## Plot to make sure that you end up with a reasonable painting
plot(nearctic_class_ouchtree, regimes=nearctic_oum_class_regimes)

## Now do the fitting using ouch
## OUM: AICc = 990.139, alpha = 11.405, sigma.sq = 70354.57 
nearctic_oum_class_ouch <- hansen(data=nearctic_class_ouchdata, tree=nearctic_class_ouchtree, regimes=nearctic_oum_class_regimes, sqrt.alpha=100, sigma=100)
## OU1: AICc = 996.019, alpha = 4.58, sigma.sq = 38891.98, global = 4.1727
nearctic_ou1_class_ouch <- hansen(data=nearctic_class_ouchdata, tree=nearctic_class_ouchtree, regimes=nearctic_ou1_class_regimes, sqrt.alpha=100, sigma=100)
## BM: AICc = 1009.58, sigma.sq = 18657.17
nearctic_bm_class_ouch <- brown(data=nearctic_class_ouchdata, tree=nearctic_class_ouchtree)

#save for supplement
saveRDS(nearctic_ou1_class_ouch, file ="ouch_results/nearctic_ou1_class_ouch")
saveRDS(nearctic_bm_class_ouch, file = "ouch_results/nearctic_bm_class_ouch")
saveRDS(nearctic_oum_class_ouch, file ="ouch_results/nearctic_oum_class_ouch")
```


#GMPD ouch Models of the residuals of gls(mpd ~ host pubs) by Class
```{r gmpd_evo_phyla, echo = FALSE}
## remake tree to be able to use ouch rather than OUwie
gmpd_class_ouchtree <- ape2ouch(gmpd_parasite_tree) ## convert to ouchtree object
## get the mpd data organized properly for ouch
gmpd_class_ouchdata <- rep(NA, length=gmpd_class_ouchtree@nnodes)
for (i in 1:length(gmpd_phyla_data$species))
  gmpd_class_ouchdata[which(gmpd_class_ouchtree@nodelabels==gmpd_phyla_data$species[i])] <- gmpd_phyla_data$trait[i]
gmpd_class_ouchdata <- as.data.frame(gmpd_class_ouchdata)

## You can use the code above, replacing nearctic with global, to find the last common ancestor for Palaeacanthocephala (243), Archiacanthocephala (229) Chromadorea (228) Enoplea (228) Trematoda (249) Monogenea (304) Cestoda (1). I drop those blocks of code here to reduce the amount of code.
gmpd_class_ouchtree@nodelabels%in%gmpd_phyla_data$species[which(gmpd_phyla_data$regime=="Chromadorea")] %>% which -> palae
## get their lineages and find the nodes that are in common across all lineages
gmpd_class_ouchtree@lineages[palae] -> palae_lineages
palae_lineages[[1]][sapply(palae_lineages[[1]], function(n) lapply(palae_lineages, function(l) n%in%l) %>% unlist %>% all) %>% which %>% min] ## 85

## paint the taxonomy-based regimes onto the tree
paint(gmpd_class_ouchtree, subtree=c("1"="Cestoda","249"= "Trematoda","228"= "Chromadorea", "243"="Palaeacanthocephala", "228" = "Enoplea","304" = "Monogenea", "229"="Archiacanthocephala" ), branch=c("1"="Cestoda")) -> gmpd_oum_class_regimes
## paint a single global regime onto the tree
paint(gmpd_class_ouchtree, subtree=c("1"="global"), branch=c("1"="global")) -> gmpd_ou1_class_regimes
## Plot to make sure that you end up with a reasonable painting
plot(gmpd_class_ouchtree, regimes=gmpd_oum_class_regimes)

## Now do the fitting using ouch
## OUM: AICc = 2415.83, alpha = 63.83, sigma.sq = 111142.2
gmpd_oum_class_ouch <- hansen(data=gmpd_class_ouchdata, tree=gmpd_class_ouchtree, regimes=gmpd_oum_class_regimes, sqrt.alpha=100, sigma=100)
## OU1: AICc = 2422.43, alpha = 80.63, sigma.sq = 150101.2, global = -0.03845
gmpd_ou1_class_ouch <- hansen(data=gmpd_class_ouchdata, tree=gmpd_class_ouchtree, regimes=gmpd_ou1_class_regimes, sqrt.alpha=100, sigma=100)
## BM: AICc = 2565.49, sigma.sq = 11552.28
gmpd_bm_class_ouch <- brown(data=gmpd_class_ouchdata, tree=gmpd_class_ouchtree)

#save for supplement 
saveRDS(gmpd_bm_class_ouch, file = "ouch_results/gmpd_bm_class_ouch")
saveRDS(gmpd_ou1_class_ouch, file = "ouch_results/gmpd_ou1_class_ouch")
saveRDS(gmpd_oum_class_ouch, file = "ouch_results/gmpd_oum_class_ouch")
```
...
#### Class regimes ####
## Simulate 1000 datasets using either the OUM, OU1 or the BM model, and then re-fit either the true data-generating model or the other model
nearctic_oum_class_datasets <- simulate(nearctic_oum_class_ouch, nsim=1000, seed=12983)
nearctic_ou1_class_datasets <- simulate(nearctic_ou1_class_ouch, nsim=1000, seed=12983)
nearctic_bm_class_datasets <- simulate(nearctic_bm_class_ouch, nsim=1000, seed=12983)
## Fit the OUM model to the OUM data
lapply(nearctic_oum_class_datasets, 
       function(data) hansen(data=data, 
                             tree=nearctic_class_ouchtree,
                             regimes=nearctic_oum_class_regimes,
                             sqrt.alpha=100, sigma=100)
) -> fit_oum_model_to_oum_data_nearctic_class
data.frame(logLik=lapply(fit_oum_model_to_oum_data_nearctic_class, function(fit) fit@loglik) %>% unlist,
           sigma.sq=lapply(fit_oum_model_to_oum_data_nearctic_class, function(fit) fit@sigma^2) %>% unlist,
           alpha=lapply(fit_oum_model_to_oum_data_nearctic_class, function(fit) fit@sqrt.alpha^2) %>% unlist,
           Acanths=lapply(fit_oum_model_to_oum_data_nearctic_class, function(fit) fit@theta[[1]][1]) %>% unlist,
           Nematodes=lapply(fit_oum_model_to_oum_data_nearctic_class, function(fit) fit@theta[[1]][2]) %>% unlist,
           Platys=lapply(fit_oum_model_to_oum_data_nearctic_class, function(fit) fit@theta[[1]][3]) %>% unlist
           ) -> fit_oum_model_to_oum_data_nearctic_class
saveRDS(fit_oum_model_to_oum_data_nearctic_class, file="bootstrap_results/fit_oum_model_to_oum_data_nearctic_class.RDS")
## Fit the OU1 model to the OUM data
lapply(nearctic_oum_class_datasets, 
       function(data) hansen(data=data, 
                             tree=nearctic_class_ouchtree,
                             regimes=nearctic_ou1_class_regimes,
                             sqrt.alpha=100, sigma=100)
) -> fit_ou1_model_to_oum_data_nearctic_class
data.frame(logLik=lapply(fit_ou1_model_to_oum_data_nearctic_class, function(fit) fit@loglik) %>% unlist,
           sigma.sq=lapply(fit_ou1_model_to_oum_data_nearctic_class, function(fit) fit@sigma^2) %>% unlist,
           alpha=lapply(fit_ou1_model_to_oum_data_nearctic_class, function(fit) fit@sqrt.alpha^2) %>% unlist,
           global=lapply(fit_ou1_model_to_oum_data_nearctic_class, function(fit) fit@theta %>% unlist) %>% unlist
           ) -> fit_ou1_model_to_oum_data_nearctic_class
saveRDS(fit_ou1_model_to_oum_data_nearctic_class, file="bootstrap_results/fit_ou1_model_to_oum_data_nearctic_class.RDS")
## Fit the BM model to the OUM data
lapply(nearctic_oum_class_datasets, 
       function(data) brown(data=data, 
                            tree=nearctic_class_ouchtree)
) -> fit_bm_model_to_oum_data_nearctic_class
data.frame(logLik=lapply(fit_bm_model_to_oum_data_nearctic_class, function(fit) fit@loglik) %>% unlist,
           sigma.sq=lapply(fit_bm_model_to_oum_data_nearctic_class, function(fit) fit@sigma^2) %>% unlist) -> fit_bm_model_to_oum_data_nearctic_class
saveRDS(fit_bm_model_to_oum_data_nearctic_class, file="bootstrap_results/fit_bm_model_to_oum_data_nearctic_class.RDS")
## Fit the OUM model to the OU1 data
lapply(nearctic_ou1_class_datasets, 
       function(data) hansen(data=data, 
                             tree=nearctic_class_ouchtree,
                             regimes=nearctic_oum_class_regimes,
                             sqrt.alpha=100, sigma=100)
) -> fit_oum_model_to_ou1_data_nearctic_class
data.frame(logLik=lapply(fit_oum_model_to_ou1_data_nearctic_class, function(fit) fit@loglik) %>% unlist,
           sigma.sq=lapply(fit_oum_model_to_ou1_data_nearctic_class, function(fit) fit@sigma^2) %>% unlist,
           alpha=lapply(fit_oum_model_to_ou1_data_nearctic_class, function(fit) fit@sqrt.alpha^2) %>% unlist,
           Acanths=lapply(fit_oum_model_to_ou1_data_nearctic_class, function(fit) fit@theta[[1]][1]) %>% unlist,
           Nematodes=lapply(fit_oum_model_to_ou1_data_nearctic_class, function(fit) fit@theta[[1]][2]) %>% unlist,
           Platys=lapply(fit_oum_model_to_ou1_data_nearctic_class, function(fit) fit@theta[[1]][3]) %>% unlist
           ) -> fit_oum_model_to_ou1_data_nearctic_class
saveRDS(fit_oum_model_to_ou1_data_nearctic_class, file="bootstrap_results/fit_oum_model_to_ou1_data_nearctic_class.RDS")
...
lapply(nearctic_ou1_class_datasets, 
       function(data) hansen(data=data, 
                             ...
                             regimes=nearctic_ou1_class_regimes,
                             sqrt.alpha=100, sigma=100)
) -> fit_ou1_model_to_ou1_data_nearctic_class
data.frame(logLik=lapply(fit_ou1_model_to_ou1_data_nearctic_class, function(fit) fit@loglik) %>% unlist,
           sigma.sq=lapply(fit_ou1_model_to_ou1_data_nearctic_class, function(fit) fit@sigma^2) %>% unlist,
           alpha=lapply(fit_ou1_model_to_ou1_data_nearctic_class, function(fit) fit@sqrt.alpha^2) %>% unlist,
           global=lapply(fit_ou1_model_to_ou1_data_nearctic_class, function(fit) fit@theta %>% unlist) %>% unlist
           ) -> fit_ou1_model_to_ou1_data_nearctic_class
saveRDS(fit_ou1_model_to_ou1_data_nearctic_class, file="bootstrap_results/fit_ou1_model_to_ou1_data_nearctic_class.RDS")

## Fit the OUM model to the BM data
lapply(nearctic_bm_class_datasets, 
       function(data) hansen(data=data, 
                             tree=nearctic_class_ouchtree,
                             regimes=nearctic_oum_class_regimes,
                             sqrt.alpha=100, sigma=100)
) -> fit_oum_model_to_bm_data_nearctic_class
data.frame(logLik=lapply(fit_oum_model_to_bm_data_nearctic_class, function(fit) fit@loglik) %>% unlist,
           sigma.sq=lapply(fit_oum_model_to_bm_data_nearctic_class, function(fit) fit@sigma^2) %>% unlist,
           alpha=lapply(fit_oum_model_to_bm_data_nearctic_class, function(fit) fit@sqrt.alpha^2) %>% unlist,
           Acanths=lapply(fit_oum_model_to_bm_data_nearctic_class, function(fit) fit@theta[[1]][1]) %>% unlist,
           Nematodes=lapply(fit_oum_model_to_bm_data_nearctic_class, function(fit) fit@theta[[1]][2]) %>% unlist,
           Platys=lapply(fit_oum_model_to_bm_data_nearctic_class, function(fit) fit@theta[[1]][3]) %>% unlist
           ) -> fit_oum_model_to_bm_data_nearctic_class
saveRDS(fit_oum_model_to_bm_data_nearctic_class, file="bootstrap_results/fit_oum_model_to_bm_data_nearctic_class.RDS")

## Fit the BM model to the BM data
lapply(nearctic_bm_class_datasets, 
       function(data) brown(data=data, 
                            tree=nearctic_class_ouchtree)
) -> fit_bm_model_to_bm_data_nearctic_class
data.frame(logLik=lapply(fit_bm_model_to_bm_data_nearctic_class, function(fit) fit@loglik) %>% unlist,
           sigma.sq=lapply(fit_bm_model_to_bm_data_nearctic_class, function(fit) fit@sigma^2) %>% unlist) -> fit_bm_model_to_bm_data_nearctic_class
saveRDS(fit_bm_model_to_bm_data_nearctic_class, file="bootstrap_results/fit_bm_model_to_bm_data_nearctic_class.RDS")


## BOOTSTRAP ANALYSIS OF GMPD DATA
## Simulate 1000 datasets using either the OUM, OU1 or the BM model, and then re-fit either the true data-generating model or the other model
gmpd_oum_class_datasets <- simulate(gmpd_oum_class_ouch, nsim=1000, seed=12983)
gmpd_ou1_class_datasets <- simulate(gmpd_ou1_class_ouch, nsim=1000, seed=12983)
gmpd_bm_class_datasets <- simulate(gmpd_bm_class_ouch, nsim=1000, seed=12983)
lapply(gmpd_oum_class_datasets, 
       function(data) hansen(data=data, 
                             tree=gmpd_class_ouchtree,
                             regimes=gmpd_oum_class_regimes,
                             sqrt.alpha=100, sigma=100)
) -> fit_oum_model_to_oum_data_gmpd_class
data.frame(logLik=lapply(fit_oum_model_to_oum_data_gmpd_class, function(fit) fit@loglik) %>% unlist,
           sigma.sq=lapply(fit_oum_model_to_oum_data_gmpd_class, function(fit) fit@sigma^2) %>% unlist,
           alpha=lapply(fit_oum_model_to_oum_data_gmpd_class, function(fit) fit@sqrt.alpha^2) %>% unlist,
           Acanths=lapply(fit_oum_model_to_oum_data_gmpd_class, function(fit) fit@theta[[1]][1]) %>% unlist,
           Nematodes=lapply(fit_oum_model_to_oum_data_gmpd_class, function(fit) fit@theta[[1]][2]) %>% unlist,
           Platys=lapply(fit_oum_model_to_oum_data_gmpd_class, function(fit) fit@theta[[1]][3]) %>% unlist
           ) -> fit_oum_model_to_oum_data_gmpd_class
saveRDS(fit_oum_model_to_oum_data_gmpd_class, file="bootstrap_results/fit_oum_model_to_oum_data_gmpd_class.RDS")

## Fit the OU1 model to the OUM data
lapply(gmpd_oum_class_datasets, 
       function(data) hansen(data=data, 
                             tree=gmpd_class_ouchtree,
                             regimes=gmpd_ou1_class_regimes,
                             sqrt.alpha=100, sigma=100)
) -> fit_ou1_model_to_oum_data_gmpd_class
data.frame(logLik=lapply(fit_ou1_model_to_oum_data_gmpd_class, function(fit) fit@loglik) %>% unlist,
           sigma.sq=lapply(fit_ou1_model_to_oum_data_gmpd_class, function(fit) fit@sigma^2) %>% unlist,
           alpha=lapply(fit_ou1_model_to_oum_data_gmpd_class, function(fit) fit@sqrt.alpha^2) %>% unlist,
           global=lapply(fit_ou1_model_to_oum_data_gmpd_class, function(fit) fit@theta %>% unlist) %>% unlist
           ) -> fit_ou1_model_to_oum_data_gmpd_class
saveRDS(fit_ou1_model_to_oum_data_gmpd_class, file="bootstrap_results/fit_ou1_model_to_oum_data_gmpd_class.RDS")

## Fit the BM model to the OUM data
lapply(gmpd_oum_class_datasets, 
       function(data) brown(data=data, 
                            tree=gmpd_class_ouchtree)
) -> fit_bm_model_to_oum_data_gmpd_class
data.frame(logLik=lapply(fit_bm_model_to_oum_data_gmpd_class, function(fit) fit@loglik) %>% unlist,
           sigma.sq=lapply(fit_bm_model_to_oum_data_gmpd_class, function(fit) fit@sigma^2) %>% unlist) -> fit_bm_model_to_oum_data_gmpd_class
saveRDS(fit_bm_model_to_oum_data_gmpd_class, file="bootstrap_results/fit_bm_model_to_oum_data_gmpd_class.RDS")

## Fit the OUM model to the OU1 data
lapply(gmpd_ou1_class_datasets, 
       function(data) hansen(data=data, 
                             tree=gmpd_class_ouchtree,
                             regimes=gmpd_oum_class_regimes,
                             sqrt.alpha=100, sigma=100)
) -> fit_oum_model_to_ou1_data_gmpd_class
data.frame(logLik=lapply(fit_oum_model_to_ou1_data_gmpd_class, function(fit) fit@loglik) %>% unlist,
           sigma.sq=lapply(fit_oum_model_to_ou1_data_gmpd_class, function(fit) fit@sigma^2) %>% unlist,
           alpha=lapply(fit_oum_model_to_ou1_data_gmpd_class, function(fit) fit@sqrt.alpha^2) %>% unlist,
           Acanths=lapply(fit_oum_model_to_ou1_data_gmpd_class, function(fit) fit@theta[[1]][1]) %>% unlist,
           Nematodes=lapply(fit_oum_model_to_ou1_data_gmpd_class, function(fit) fit@theta[[1]][2]) %>% unlist,
           Platys=lapply(fit_oum_model_to_ou1_data_gmpd_class, function(fit) fit@theta[[1]][3]) %>% unlist
           ) -> fit_oum_model_to_ou1_data_gmpd_class
saveRDS(fit_oum_model_to_ou1_data_gmpd_class, file="bootstrap_results/fit_oum_model_to_ou1_data_gmpd_class.RDS")
...
lapply(gmpd_ou1_class_datasets, 
       function(data) hansen(data=data, 
                             ...
                             regimes=gmpd_ou1_class_regimes,
                             sqrt.alpha=100, sigma=100)
) -> fit_ou1_model_to_ou1_data_gmpd_class
data.frame(logLik=lapply(fit_ou1_model_to_ou1_data_gmpd_class, function(fit) fit@loglik) %>% unlist,
           sigma.sq=lapply(fit_ou1_model_to_ou1_data_gmpd_class, function(fit) fit@sigma^2) %>% unlist,
           alpha=lapply(fit_ou1_model_to_ou1_data_gmpd_class, function(fit) fit@sqrt.alpha^2) %>% unlist,
           global=lapply(fit_ou1_model_to_ou1_data_gmpd_class, function(fit) fit@theta %>% unlist) %>% unlist
           ) -> fit_ou1_model_to_ou1_data_gmpd_class
saveRDS(fit_ou1_model_to_ou1_data_gmpd_class, file="bootstrap_results/fit_ou1_model_to_ou1_data_gmpd_class.RDS")

## Fit the OUM model to the BM data
lapply(gmpd_bm_class_datasets, 
       function(data) hansen(data=data, 
                             tree=gmpd_class_ouchtree,
                             regimes=gmpd_oum_class_regimes,
                             sqrt.alpha=100, sigma=100)
) -> fit_oum_model_to_bm_data_gmpd_class
data.frame(logLik=lapply(fit_oum_model_to_bm_data_gmpd_class, function(fit) fit@loglik) %>% unlist,
           sigma.sq=lapply(fit_oum_model_to_bm_data_gmpd_class, function(fit) fit@sigma^2) %>% unlist,
           alpha=lapply(fit_oum_model_to_bm_data_gmpd_class, function(fit) fit@sqrt.alpha^2) %>% unlist,
           Acanths=lapply(fit_oum_model_to_bm_data_gmpd_class, function(fit) fit@theta[[1]][1]) %>% unlist,
           Nematodes=lapply(fit_oum_model_to_bm_data_gmpd_class, function(fit) fit@theta[[1]][2]) %>% unlist,
           Platys=lapply(fit_oum_model_to_bm_data_gmpd_class, function(fit) fit@theta[[1]][3]) %>% unlist
           ) -> fit_oum_model_to_bm_data_gmpd_class
saveRDS(fit_oum_model_to_bm_data_gmpd_class, file="bootstrap_results/fit_oum_model_to_bm_data_gmpd_class.RDS")

## Fit the BM model to the BM data
lapply(gmpd_bm_class_datasets, 
       function(data) brown(data=data, 
                            tree=gmpd_class_ouchtree)
) -> fit_bm_model_to_bm_data_gmpd_class
data.frame(logLik=lapply(fit_bm_model_to_bm_data_gmpd_class, function(fit) fit@loglik) %>% unlist,
           sigma.sq=lapply(fit_bm_model_to_bm_data_gmpd_class, function(fit) fit@sigma^2) %>% unlist) -> fit_bm_model_to_bm_data_gmpd_class
saveRDS(fit_bm_model_to_bm_data_gmpd_class, file="bootstrap_results/fit_bm_model_to_bm_data_gmpd_class.RDS")
...
fit_oum_model_to_oum_data_nearctic_class <- readRDS(file="bootstrap_results/fit_oum_model_to_oum_data_nearctic_class.RDS")
fit_oum_model_to_ou1_data_nearctic_class <- readRDS(file="bootstrap_results/fit_oum_model_to_ou1_data_nearctic_class.RDS")
fit_oum_model_to_bm_data_nearctic_class <- readRDS(file="bootstrap_results/fit_oum_model_to_bm_data_nearctic_class.RDS")
fit_ou1_model_to_oum_data_nearctic_class <- readRDS(file="bootstrap_results/fit_ou1_model_to_oum_data_nearctic_class.RDS")
fit_ou1_model_to_ou1_data_nearctic_class <- readRDS(file="bootstrap_results/fit_ou1_model_to_ou1_data_nearctic_class.RDS")
fit_bm_model_to_oum_data_nearctic_class <- readRDS(file="bootstrap_results/fit_bm_model_to_oum_data_nearctic_class.RDS")
fit_bm_model_to_bm_data_nearctic_class <- readRDS(file="bootstrap_results/fit_bm_model_to_bm_data_nearctic_class.RDS")


fit_oum_model_to_oum_data_gmpd_class <- readRDS(file="bootstrap_results/fit_oum_model_to_oum_data_gmpd_class.RDS")
fit_oum_model_to_ou1_data_gmpd_class <- readRDS(file="bootstrap_results/fit_oum_model_to_ou1_data_gmpd_class.RDS")
fit_oum_model_to_bm_data_gmpd_class <- readRDS(file="bootstrap_results/fit_oum_model_to_bm_data_gmpd_class.RDS")
fit_ou1_model_to_oum_data_gmpd_class <- readRDS(file="bootstrap_results/fit_ou1_model_to_oum_data_gmpd_class.RDS")
fit_ou1_model_to_ou1_data_gmpd_class <- readRDS(file="bootstrap_results/fit_ou1_model_to_ou1_data_gmpd_class.RDS")
fit_bm_model_to_oum_data_gmpd_class <- readRDS(file="bootstrap_results/fit_bm_model_to_oum_data_gmpd_class.RDS")
fit_bm_model_to_bm_data_gmpd_class <- readRDS(file="bootstrap_results/fit_bm_model_to_bm_data_gmpd_class.RDS")
...
(((fit_bm_model_to_bm_data_gmpd_class$logLik-fit_oum_model_to_bm_data_gmpd_class$logLik) < (gmpd_bm_class_ouch@loglik - gmpd_oum_class_ouch@loglik)) %>% sum)/nrow(fit_bm_model_to_bm_data_gmpd_class) -> p_value_bm_vs_oum_gmpd_class #0 

(((fit_bm_model_to_bm_data_nearctic_class$logLik-fit_oum_model_to_bm_data_nearctic_class$logLik) < (nearctic_bm_class_ouch@loglik - nearctic_oum_class_ouch@loglik)) %>% sum)/nrow(fit_bm_model_to_bm_data_nearctic_class) -> p_value_bm_vs_oum_nearctic_class #0
...
(((fit_ou1_model_to_ou1_data_gmpd_class$logLik-fit_oum_model_to_ou1_data_gmpd_class$logLik) < (gmpd_ou1_class_ouch@loglik - gmpd_oum_class_ouch@loglik)) %>% sum)/nrow(fit_ou1_model_to_ou1_data_gmpd_class) -> p_value_ou1_vs_oum_gmpd_class #0.012

(((fit_ou1_model_to_ou1_data_nearctic_class$logLik-fit_oum_model_to_ou1_data_nearctic_class$logLik) < (nearctic_ou1_class_ouch@loglik - nearctic_oum_class_ouch@loglik)) %>% sum)/nrow(fit_ou1_model_to_ou1_data_nearctic_class) -> p_value_ou1_vs_oum_nearctic_class #0.013 
...
(((-2*(fit_bm_model_to_oum_data_gmpd_class$logLik - fit_oum_model_to_oum_data_gmpd_class$logLik)) > sort(-2*(fit_bm_model_to_bm_data_gmpd_class$logLik-fit_oum_model_to_bm_data_gmpd_class$logLik))[950]) %>% sum)/1000 -> power_to_reject_bm_gmpd_class #1

(((-2*(fit_bm_model_to_oum_data_nearctic_class$logLik - fit_oum_model_to_oum_data_nearctic_class$logLik)) > sort(-2*(fit_bm_model_to_bm_data_nearctic_class$logLik-fit_oum_model_to_bm_data_nearctic_class$logLik))[950]) %>% sum)/1000 -> power_to_reject_bm_nearctic_class #0.999
...
(((-2*(fit_ou1_model_to_oum_data_gmpd_class$logLik - fit_oum_model_to_oum_data_gmpd_class$logLik)) > sort(-2*(fit_ou1_model_to_ou1_data_gmpd_class$logLik-fit_oum_model_to_ou1_data_gmpd_class$logLik))[950]) %>% sum)/1000 -> power_to_reject_ou1_gmpd_class # 0.862

(((-2*(fit_ou1_model_to_oum_data_nearctic_class$logLik - fit_oum_model_to_oum_data_nearctic_class$logLik)) > sort(-2*(fit_ou1_model_to_ou1_data_nearctic_class$logLik-fit_oum_model_to_ou1_data_nearctic_class$logLik))[950]) %>% sum)/1000 -> power_to_reject_ou1_nearctic_class # 0.916
```
...
mtext(side=3, line=0.25, "GMPD (regimes by Phyla)")
...
mtext(side=1, line=2, expression(delta))
mtext(side=2, line=2, 'Density')
mtext(side=3, line=0.25, "Nearctic (regimes by Phyla)")
lines(d1$x, d1$y, col=gray(0.7))
with(d1, polygon(x=c(min(x), x[x<max(x)], rev(x[x<max(x)]), min(x)), y=c(0, 0*x[x<max(x)], rev(y[x<max(x)]), 0), col=gray(0.7, alpha=0.5)))
lines(d2$x, d2$y, col=gray(0.2))
with(d2, polygon(x=c(min(x), x[x<max(x)], rev(x[x<max(x)]), min(x)), y=c(0, 0*x[x<max(x)], rev(y[x<max(x)]), 0), col=gray(0.2, alpha=0.5)))
abline(v=delta, lwd=2, lty=2)
#GMPD by Class
delta <- -2*(gmpd_bm_class_ouch@loglik - gmpd_oum_class_ouch@loglik)
delta.dist <- -2*(fit_bm_model_to_bm_data_gmpd_class$logLik - fit_oum_model_to_bm_data_gmpd_class$logLik)
delta.dist.2 <- -2*(fit_bm_model_to_oum_data_gmpd_class$logLik - fit_oum_model_to_oum_data_gmpd_class$logLik)
d1 <- density(delta.dist)
d2 <- density(delta.dist.2)
plot.new()
plot.window(xlim=range(c(d1$x,d2$x)), ylim=range(c(d1$y,d2$y)))
...
mtext(side=1, line=2, expression(delta))
mtext(side=2, line=2, 'Density')
mtext(side=3, line=0.25, "GMPD (regimes by Class)")
lines(d1$x, d1$y, col=gray(0.7))
with(d1, polygon(x=c(min(x), x[x<max(x)], rev(x[x<max(x)]), min(x)), y=c(0, 0*x[x<max(x)], rev(y[x<max(x)]), 0), col=gray(0.7, alpha=0.5)))
lines(d2$x, d2$y, col=gray(0.2))
with(d2, polygon(x=c(min(x), x[x<max(x)], rev(x[x<max(x)]), min(x)), y=c(0, 0*x[x<max(x)], rev(y[x<max(x)]), 0), col=gray(0.2, alpha=0.5)))
...
legend(x='topright', c('BM','OU3'), fill=c(gray(0.7),gray(0.2)), bty='n', cex=0.75)
#Nearctic by Class
delta <- -2*(nearctic_bm_class_ouch@loglik - nearctic_oum_class_ouch@loglik)
delta.dist <- -2*(fit_bm_model_to_bm_data_nearctic_class$logLik - fit_oum_model_to_bm_data_nearctic_class$logLik)
delta.dist.2 <- -2*(fit_bm_model_to_oum_data_nearctic_class$logLik - fit_oum_model_to_oum_data_nearctic_class$logLik)
d1 <- density(delta.dist)
d2 <- density(delta.dist.2)
plot.new()
plot.window(xlim=range(c(d1$x,d2$x)), ylim=range(c(d1$y,d2$y)))
...
mtext(side=1, line=2, expression(delta))
mtext(side=2, line=2, 'Density')
mtext(side=3, line=0.25, "Nearctic (regimes by Class)")
lines(d1$x, d1$y, col=gray(0.7))
with(d1, polygon(x=c(min(x), x[x<max(x)], rev(x[x<max(x)]), min(x)), y=c(0, 0*x[x<max(x)], rev(y[x<max(x)]), 0), col=gray(0.7, alpha=0.5)))
lines(d2$x, d2$y, col=gray(0.2))
with(d2, polygon(x=c(min(x), x[x<max(x)], rev(x[x<max(x)]), min(x)), y=c(0, 0*x[x<max(x)], rev(y[x<max(x)]), 0), col=gray(0.2, alpha=0.5)))
...
abline(v=delta, lwd=2, lty=2)
## GMPD by Class
delta <- -2*(gmpd_ou1_class_ouch@loglik - gmpd_oum_class_ouch@loglik)
delta.dist <- -2*(fit_ou1_model_to_ou1_data_gmpd_class$logLik - fit_oum_model_to_ou1_data_gmpd_class$logLik)
delta.dist.2 <- -2*(fit_ou1_model_to_oum_data_gmpd_class$logLik - fit_oum_model_to_oum_data_gmpd_class$logLik)
d1 <- density(delta.dist)
d2 <- density(delta.dist.2)
plot.new()
plot.window(xlim=range(c(d1$x,d2$x)), ylim=range(c(d1$y,d2$y)))
...
legend(x='topright', c('OU1','OU3'), fill=c(gray(0.7),gray(0.2)), bty='n', cex=0.75)
#Nearctic by Class
delta <- -2*(nearctic_ou1_class_ouch@loglik - nearctic_oum_class_ouch@loglik)
delta.dist <- -2*(fit_ou1_model_to_ou1_data_nearctic_class$logLik - fit_oum_model_to_ou1_data_nearctic_class$logLik)
delta.dist.2 <- -2*(fit_ou1_model_to_oum_data_nearctic_class$logLik - fit_oum_model_to_oum_data_nearctic_class$logLik)
d1 <- density(delta.dist)
d2 <- density(delta.dist.2)
plot.new()
plot.window(xlim=range(c(d1$x,d2$x)), ylim=range(c(d1$y,d2$y)))
...
tibble(model=c("BM","OU1","OU3"),
       aicc=c(summary(nearctic_bm_class_ouch)$aic.c, 
              summary(nearctic_ou1_class_ouch)$aic.c,
              summary(nearctic_oum_class_ouch)$aic.c)
       ) %>% 
  mutate(deltaaicc=aicc-min(aicc), 
         pow=exp(-.5*deltaaicc),
         sumpow=sum(pow),
         waicc=pow/sumpow) %>%
  dplyr::select(c(1,2,3,6)) %>%
  arrange(desc(waicc)) -> nearctic_aic_OU_class
nearctic_aic_OU_class
...
tibble(model=c("BM","OU1","OU3"),
       aicc=c(summary(gmpd_bm_class_ouch)$aic.c, 
              summary(gmpd_ou1_class_ouch)$aic.c,
              summary(gmpd_oum_class_ouch)$aic.c)
       ) %>% 
  mutate(deltaaicc=aicc-min(aicc), 
         pow=exp(-.5*deltaaicc),
         sumpow=sum(pow),
         waicc=pow/sumpow) %>%
  dplyr::select(c(1,2,3,6)) %>%
  arrange(desc(waicc)) -> gmpd_aic_OU_class
gmpd_aic_OU_class
...
write.csv(b.size, "data/bodysizepara.csv")
...
#load mammal data
Trait_data <- read_csv("data/Trait_data.csv")
...
#90 taxa (AICc = 994.993)
summary(nearctic_trans_mod<-pgls(res ~  close + env + intermediate, cdat, lambda = "ML")) #Intermediate ***
...
#all predictors (28 taxa)
nearctic_mod <- pgls(res ~ close + env + intermediate + para.b + host_mass, nearctic_cdat, lambda = "ML")
summary(nearctic_mod)  #AICc = 329.4071
anova(nearctic_mod) #environmental transmission barely
```
...
# 218 taxa (AICc = 2126.037)
summary(gmpd_trans_mod<-pgls(res ~  close + env + intermediate, gmpd_cdat, lambda = "ML")) #Environmental *** and Intermediate**
...
## model testing ##
#all predictors (58 taxa; AICc = 555.1904)
summary(gmpd_mod<-pgls(res ~ close + env + intermediate + para.b + host_mass, gmpd_trait_cdat, lambda = "ML"))
