...
Do not use (below) in the MS or include in the GitHub code
```{r evolutionary_models_for_MPD, include = FALSE}
...
# plot distances2 ~ global.mam.mpd
plot(distance2 ~ global.mam.mpd)


figure_data<-data.frame("name"= names(global.mam.mpd), "global.mpd" = global.mam.mpd, "distance"= distance2)

figure_data %>%
  separate(name, c("Genus", "species"), " ", remove= F) ->figure_data
...
####What helminths infect the "zoonotic" data point?####
global_df %>%
  as.data.frame ->help
